...
| B2 | 多通道通知触达 + 滞留转化 | 试验期不走商用短信；需真实触达演示 | notify / overdue | 强必要：串起 notify+overdue | 🟡 partial（免费通道+绑定+重发+M53 单件补发/记录筛选；商用短信不做） |
...
> **首推顺序建议**：B1–B5、B6 与通知/入库运营（M50–M56）已落地。按 `docs/TRIAL-CHECKLIST.md` 做真实试用。
