...
| B2 | 多通道通知触达 + 滞留转化 | 试验期不走商用短信；需真实触达演示 | notify / overdue | 强必要：串起 notify+overdue | 🟡 partial（免费通道 + 客户扫码/备选绑定 + 入库/批量触达反馈 + 管理端重发已落地；商用短信明确不做） |
| B3 | 取件人身份核验 + 冒领留证 | 取件码可转发/偷看，晚高峰拿错件、冒领是真实纠纷源 | outbound / scan（PRD §4.5 出库两步流程） | 必要：降低纠纷与责任风险 | ✅ done（后4位核验 + 可选拍照 + 可选取件签名/大件推荐） |
| B4 | 到付件 + 代收货款（对用户收款线） | 到付、代收货款是真实现金业务，当前 finance 只做「与快递公司月结」，对用户收款缺失，钱账对不上 | finance / inbound / outbound | 必要：补齐现金流闭环 | 🟡 partial（入库录金额 + 出库收款确认 + 日结/待办；与快递公司月结独立） |
...
> **首推顺序建议**：B1–B3 已落地；B4 对用户收款第一刀已落地。下一优先 **部署准备** 或 **B4 免收/改价/日结导出** 或 B5/B6。
