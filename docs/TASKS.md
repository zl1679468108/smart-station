...
| B6 | 取件用户主动侧（订阅提醒 + 预约取件 + 到店导航） | 「有没有我的件」靠用户自查；跑空与滞留高 | query 门户 / notify | 有价值：提升留存、降跑空 | 🟡 partial（M50 导航+预约；M51 预约触达+工作台待办；订阅仍靠绑定通道） |

> **首推顺序建议**：B1–B5 已落地；B6 导航+预约+触达（M50/M51）已落地。下一优先 **部署/试用打磨** 或时段可配置。
