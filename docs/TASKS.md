...
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M115.1 | P1 | 库存页「打印本页小票」 | inventory | done | 适合货架筛选后补打 |

## M116 出库取件回执打印（业务功能）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M116.1 | P0 | printOutboundReceipt | print util | done | 脱敏回执 |
| M116.2 | P0 | 人工出库成功打印 | outbound | done | 回执区按钮 |
| M116.3 | P1 | 扫描机成功打印 | scan | done | |

## M117 交班复用今日跟进摘要（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M117.1 | P0 | 抽取 dailyFollowup 共用工具 | utils | done | 清单+摘要文案 |
| M117.2 | P0 | 交班页接入 DailyFollowupCard | shifts | done | 含本班快照 |
| M117.3 | P1 | 交班确认弹窗复制摘要 | shifts modal | done | |
...
## M129 交班优先补发失败 + 批量入库拆分失败补发（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M129.1 | P0 | 今日跟进：私信失败优先+发送失败项 | dailyFollowup | done | days=1 深链 |
...
## M131 通知失败优先落地 + 库存详情再发（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M131.1 | P0 | 通知页无深链时失败优先自动筛 | NotifyTab | done | 私信失败>发送失败>未绑定 |
| M131.2 | P0 | 库存详情补发/滞留失败可再发 | inventory Detail | done | 回执含状态 |

## M132 工作台失败优先快捷入口 + 预约/寄件失败路径（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M132.1 | P0 | 今日跟进卡主按钮补发私信失败 | DailyFollowupCard | done | |
| M132.2 | P0 | 工作台深链 days=1 + 动态优先失败 | Dashboard | done | |
| M132.3 | P1 | 预约发送失败回执可去补发 | appointments | done | |
| M132.4 | P2 | 寄件页补发失败私信入口 | shipping | done | |

## M133 异常失败再发 + 按手机号一键补发（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M133.1 | P0 | 异常页补发回执失败可再发 | exception | done | |
...
## M136 库存失败再发回执 + smoke 批量路由（运营打磨）
| 编号 | 优先级 | 任务 | 范围 | 状态 | 备注 |
|------|--------|------|------|------|------|
| M136.1 | P0 | 库存补发/滞留回执结构化 + 一键再发失败 | Inventory | done | 对齐滞留页 |
