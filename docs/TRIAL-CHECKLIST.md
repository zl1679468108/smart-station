...
3. 入库（扫码/手动）：
   ...
   - 成功区/最近列表可「看包裹」进库存详情
4. 未绑定客户：查件页点「现在绑定/扫一扫收码」；绑定后入库页可「补发通知」；批量可「一键补发未私信」  
5. 工作台「今日到件触达」→ 点整卡看今日；点「未绑定/已私信/私信失败」进对应筛选  
6. 通知记录：顶部今日触达漏斗可点选；筛选 今日 / 发送失败 / 到件 / 滞留 / 未私信 / 已私信 / 私信失败；本页可一键补发未私信；有关联包裹时点「看包裹」进库存详情  
...
13. 批量入库：支持 Excel 粘贴/表头/+86；可「仅预检」或「预检并导入」；预检/成功/失败可导出；成功清单可筛选未私信/已私信，点取件码或「看包裹」进详情，可跳通知记录  
...
| 滞留 | 立即扫描；滞留页/库存列表/详情可发提醒；库存可勾选批量提醒 |
| 预约 | 查件页预约成功可绑定；「我的预约」状态白话；后台代客/确认有回执 |
| 交接班 | 开班 → 入出库 → 交班快照 |
| 财务 | 收款日结与快递公司月结分开 |
