...
3. 入库（扫码/手动）：
   ...
   - 成功区/最近列表可「看包裹」进库存详情
4. 未绑定客户：店员可复制「绑定链接话术」一对一发给客户；查件有在库件时自动展开绑定区；结果页可复制取件码/全部码 +「两步取件·绑定后自动收码」+ 包裹卡「绑定后可微信收此码」；全端底部固定绑定条；未查到也可先绑定；**绑定成功后自动给在库包裹私信取件码**并 toast 补发件数（近 6 小时已私信的不重复）；入库页可「补发通知」；批量可「一键补发未私信」  
