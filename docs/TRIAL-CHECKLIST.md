...
9. 通知记录：分页 + 导出 CSV；「按手机号」聚合未私信客户；可一键复制未绑定跟进清单（含手机号，勿发群）  
...
| 统计 | 可见件次私信率/人数覆盖/今日新绑；可复制/导出今日未绑定清单；漏斗/滞留白话说明 |
| 工作台/大屏 | 今日触达含件次私信率/人数覆盖/今日新绑；动态可显示已私信/未绑定；未绑定可跳按手机跟进；大屏有到件触达速览 |
