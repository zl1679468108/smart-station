...
test.describe('手机号查询', () => {
  ...
  test('查询无结果显示空状态', async ({ page }) => {
    ...
    await expect(page.getByText('未查询到可取件包裹')).toBeVisible({ timeout: 8000 });
  });
});
...
test.describe('确认出库流程', () => {
  test('点击确认出库弹出二次确认弹窗', async ({ page }) => {
    ...
    await expect(page.getByText('确认出库 · 身份核验')).toBeVisible();
    await expect(page.getByText(/防冒领/)).toBeVisible();
    await expect(page.getByPlaceholder('向取件人询问后填写')).toBeVisible();
    await expect(page.getByRole('button', { name: '取消' })).toBeVisible();
    await expect(page.getByRole('button', { name: '核验并出库' })).toBeVisible();
  });
  ...
  test('取消确认关闭弹窗', async ({ page }) => {
    ...
    await expect(page.getByText('确认出库 · 身份核验')).toBeHidden();
  });
  ...
  test('确认后出库成功从列表移除', async ({ page }) => {
    ...
    // 成功 Toast
    await expect(page.getByText('包裹已出库', { exact: true })).toBeVisible({ timeout: 8000 });
    // 列表中应移除该包裹
    await expect(page.getByText('找到 1 个可取件包裹')).toBeHidden();
  });
